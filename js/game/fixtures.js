...
import { pathFixtures, FixtureBehaviorType, getFixtureTypeForPath } from "../data/pathFixtures.js";
import { getConnectedEntity, updateFixtureConnections } from "./connections.js";
import { findSegmentContainingPoint, getPathSegments, findPerpendicularCrossing } from "./geometry.js";
 ...
 */
function propagateAcrossBridges(path, fromEndpoint, fieldState, visitedPaths = new Set()) {
    // Prevent cycles
    ...
    if ('side' in connected) {
        ...
        const bridgePosition = [fixture.x, fixture.y];
        ...
        const oppositeStartAtBridge = oppositeStartPoint[0] === bridgePosition[0] &&
            oppositeStartPoint[1] === bridgePosition[1];
        const oppositeEndAtBridge = oppositeEndPoint[0] === bridgePosition[0] &&
            oppositeEndPoint[1] === bridgePosition[1];
        // Determine which endpoint to continue propagating from
        let oppositeFromEndpoint;
 ...
 */
function getPathOtherEndConnectionType(path, bridgeSidePosition, fieldState) {
 ...
 */
export function calculateBridgeFlows(fixture, fieldState) {
    // Get the bridge position
    const bridgePos = [fixture.x, fixture.y];
    ...
    function analyzeAxis(side1, side1Dir, side2, side2Dir) {
 ...
 */
export function calculateSplitterFlows(fixture, fieldState) {
    const inputFlows = [];
    ...
    const connectedOutputSides = fixture.sides.filter(side => side.subType === 'output' && side.connectedPathID !== null && side.connectedPathID !== undefined);
 ...
 * @returns Input and output flows
 */
export function calculateConvergerFlows(fixture, fieldState) {
    const allInputFlows = [];
 ...
 * @returns Input and output flows
 */
export function calculateControlPortFlows(fixture, fieldState) {
    const inputFlowArrays = [];
 ...
 */
export function calculateFixtureFlows(fixture, fieldState) {
    let inputFlows = [];
    let outputFlows = [];
    ...
    const updatedSides = fixture.sides.map(side => {
        if (fixtureDef?.behaviorType === FixtureBehaviorType.BRIDGE) {
            ...
            let sideFlows = [];
            let determinedSubType = 'output'; // Default to output if undetermined
            if (side.direction === 'left' || side.direction === 'right') {
                ...
                if (leftConnected && rightConnected) {
                    ...
                    if (leftPath && rightPath) {
                        const leftEndType = getPathOtherEndConnectionType(leftPath, [fixture.x, fixture.y], fieldState);
                        const rightEndType = getPathOtherEndConnectionType(rightPath, [fixture.x, fixture.y], fieldState);
                ...
                if (upConnected && downConnected) {
                    ...
                    if (upPath && downPath) {
                        const upEndType = getPathOtherEndConnectionType(upPath, [fixture.x, fixture.y], fieldState);
                        const downEndType = getPathOtherEndConnectionType(downPath, [fixture.x, fixture.y], fieldState);
 ...
 */
export function validateFixturePlacementOnPath(fixtureType, position, rotation, path) {
 ...
 */
export function splitPathAtFixture(path, fixtureType, position, rotation, segmentIndex) {
    const changes = [];
    const fixtureDef = pathFixtures[fixtureType];
 ...
 */
export function findPathsConnectedToFixture(fixture, fieldState) {
    const connected = [];
 ...
 */
export function reconnectPathsAfterFixtureRemoval(fixture, fieldState) {
    const changes = [];
    ...
    if (connectedPaths.length === 2) {
        ...
        for (const conn1 of connectedPaths) {
            ...
            if (conn2 && !sidePairs.some(pair => (pair[0].path.id === conn1.path.id && pair[1].path.id === conn2.path.id) ||
 ...
 */
function createPathMergeChanges(path1, path2, fixture) {
    const changes = [];
    if (path1.type !== path2.type) {
        // Can't merge paths of different types
        return changes;
    }
    const fixturePos = [fixture.x, fixture.y];
    ...
    if (path1EndsAtFixture && path2StartsAtFixture) {
        // path1 -> fixture -> path2
        mergedPoints = [...path1.points.slice(0, -1), ...path2.points.slice(1)];
    }
    else if (path1StartsAtFixture && path2EndsAtFixture) {
        // path2 -> fixture -> path1
        mergedPoints = [...path2.points.slice(0, -1), ...path1.points.slice(1)];
    }
    else if (path1EndsAtFixture && path2EndsAtFixture) {
        // path1 -> fixture <- path2 (reverse path2)
        const reversedPath2 = [...path2.points].reverse();
        mergedPoints = [...path1.points.slice(0, -1), ...reversedPath2.slice(1)];
    }
        ...
        mergedPoints = [...reversedPath1.slice(0, -1), ...path2.points.slice(1)];
    }
 ...
 */
export function generateFixtureRelocationChanges(fixture, newPosition, originalFieldState, intermediateFieldState, includeReconnection = true) {
    const changes = [];
    ...
    if (pathToSplit && segmentIndex !== undefined) {
        const splitChanges = splitPathAtFixture(pathToSplit, fixture.type, newPosition, fixture.rotation, segmentIndex);
