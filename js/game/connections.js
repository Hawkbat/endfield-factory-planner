...
import { pathFixtures, FixtureBehaviorType } from "../data/pathFixtures.js";
/**
 ...
 */
export function findPortAtPosition(facility, x, y, direction) {
 ...
 */
export function findFixtureSideAtPosition(fixture, x, y, direction) {
 ...
 */
export function getConnectedEntity(path, endpoint, fieldState) {
 ...
 */
export function preservePortProperties(oldPorts, newPorts) {
...
export function initializeFacilityPorts(facility) {
    ...
    const createPortsFromSide = (sideDef, type, subType, external) => {
        if (!sideDef)
            return;
 ...
 */
export function initializeFixtureSides(fixture) {
 ...
 */
export function calculatePathFlowDirection(path, fieldState) {
    const errorFlags = {};
    ...
    for (const facility of fieldState.facilities) {
        ...
        const startOutputSame = findPortAtPosition(facility, startPoint[0], startPoint[1], startDirection);
        ...
        const startOutputOpposite = findPortAtPosition(facility, startPoint[0], startPoint[1], getOppositeDirection(startDirection));
        ...
        const startInputOpposite = findPortAtPosition(facility, startPoint[0], startPoint[1], getOppositeDirection(startDirection));
        ...
        const startInputSame = findPortAtPosition(facility, startPoint[0], startPoint[1], startDirection);
        if (startInputSame && startInputSame.port.subType === 'input') {
            startIsInput = true;
        }
        // At end: check for output facing same direction (start-to-end flow)
        const endOutputSame = findPortAtPosition(facility, endPoint[0], endPoint[1], endDirection);
        ...
        const endOutputOpposite = findPortAtPosition(facility, endPoint[0], endPoint[1], getOppositeDirection(endDirection));
        ...
        const endInputOpposite = findPortAtPosition(facility, endPoint[0], endPoint[1], getOppositeDirection(endDirection));
        if (endInputOpposite && endInputOpposite.port.subType === 'input') {
            endIsInput = true;
        }
        // At end: check for input facing same direction (end-to-start flow)
        const endInputSame = findPortAtPosition(facility, endPoint[0], endPoint[1], endDirection);
    ...
    for (const fixture of fieldState.pathFixtures) {
        ...
        const startOutputSame = findFixtureSideAtPosition(fixture, startPoint[0], startPoint[1], startDirection);
        ...
        const startInputOpposite = findFixtureSideAtPosition(fixture, startPoint[0], startPoint[1], getOppositeDirection(startDirection));
        ...
        const endInputOpposite = findFixtureSideAtPosition(fixture, endPoint[0], endPoint[1], getOppositeDirection(endDirection));
        ...
        const endOutputSame = findFixtureSideAtPosition(fixture, endPoint[0], endPoint[1], endDirection);
 ...
 */
export function updateAllPathConnections(fieldState) {
 ...
 */
export function updatePathConnectionRefs(path, fieldState) {
    if (path.points.length < 2) {
        return { ...path, startConnectedTo: null, endConnectedTo: null };
    }
    const startPoint = path.points[0];
    const endPoint = path.points[path.points.length - 1];
    let startConnectedTo = null;
 ...
 */
export function updateFacilityConnections(facility, fieldState) {
 ...
 */
export function updateFixtureConnections(fixture, fieldState) {
