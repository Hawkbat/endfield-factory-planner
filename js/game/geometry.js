...
function isPointWithinExtendedBounds(x, y, fieldState) {
 ...
 */
export function isInBounds(x, y, width, height, fieldState) {
 ...
 */
export function doEntitiesOverlap(entity1, entity2) {
    // Check if there's NO overlap, then negate
    // No overlap if one is completely to the left, right, above, or below the other
    const noOverlap = entity1.x + entity1.width <= entity2.x || // entity1 is completely to the left
 ...
 */
export function doesPathSegmentOverlapEntity(segmentStart, segmentEnd, entity) {
 ...
 */
export function validatePathGeometry(path) {
 ...
 */
export function validateFacilityPlacement(facility, fieldState) {
    const errors = {};
    // Check bounds
 ...
 * @returns Object with error flags
 */
export function validateFixturePlacement(fixture, fieldState) {
    const errors = {};
 ...
 * @returns Object with error flags
 */
export function validatePathPlacement(path, fieldState) {
    const errors = {};
    // Check geometry
    ...
    for (const facility of fieldState.facilities) {
        for (const segment of segments) {
            ...
            if (doesPathSegmentOverlapEntity(segment.from, segment.to, facility)) {
                ...
                const startIsConnectedToThisFacility = path.startConnectedTo?.type === 'facility' &&
                    path.startConnectedTo.facilityID === facility.id;
                const endIsConnectedToThisFacility = path.endConnectedTo?.type === 'facility' &&
                ...
                const startWithinFacility = startPoint[0] >= facility.x && startPoint[0] < facility.x + facility.width &&
                    startPoint[1] >= facility.y && startPoint[1] < facility.y + facility.height;
                const endWithinFacility = endPoint[0] >= facility.x && endPoint[0] < facility.x + facility.width &&
    ...
    for (const fixture of fieldState.pathFixtures) {
        ...
        for (const segment of segments) {
            if (doesPathSegmentOverlapEntity(segment.from, segment.to, fixture)) {
                ...
                const startIsConnectedToThisFixture = path.startConnectedTo?.type === 'fixture' &&
                    path.startConnectedTo.fixtureID === fixture.id;
                const endIsConnectedToThisFixture = path.endConnectedTo?.type === 'fixture' &&
                ...
                const startPointMatchesFixture = startPoint[0] === fixture.x && startPoint[1] === fixture.y;
                const endPointMatchesFixture = endPoint[0] === fixture.x && endPoint[1] === fixture.y;
        ...
        for (const segment of segments) {
            for (const otherSegment of otherSegments) {
                ...
                if (doPathSegmentsOverlap(segment.from, segment.to, otherSegment.from, otherSegment.to)) {
                    ...
                    if (sharedPoint) {
                        const fixtureAtPoint = fieldState.pathFixtures.find(f => f.x === sharedPoint[0] && f.y === sharedPoint[1]);
                        if (fixtureAtPoint) {
 ...
 */
function findSharedEndpoint(seg1Start, seg1End, seg2Start, seg2End) {
 ...
 */
function doPathSegmentsOverlap(seg1Start, seg1End, seg2Start, seg2End) {
 ...
 */
export function isPointOnSegment(point, segmentStart, segmentEnd) {
    const [px, py] = point;
    const [x1, y1] = segmentStart;
    const [x2, y2] = segmentEnd;
 ...
 */
export function findSegmentContainingPoint(point, path) {
    const segments = getPathSegments(path);
 ...
 */
export function findPerpendicularCrossing(seg1Start, seg1End, seg2Start, seg2End) {
    const [x1Min, x1Max] = [Math.min(seg1Start[0], seg1End[0]), Math.max(seg1Start[0], seg1End[0])];
    const [y1Min, y1Max] = [Math.min(seg1Start[1], seg1End[1]), Math.max(seg1Start[1], seg1End[1])];
    const [x2Min, x2Max] = [Math.min(seg2Start[0], seg2End[0]), Math.max(seg2Start[0], seg2End[0])];
    const [y2Min, y2Max] = [Math.min(seg2Start[1], seg2End[1]), Math.max(seg2Start[1], seg2End[1])];
