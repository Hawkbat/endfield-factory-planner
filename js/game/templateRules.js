...
import { pathTypeAllowedRegions } from "../data/pathTypes.js";
...
function getTemplateDepotBusRanges(template, fieldWidth, fieldHeight) {
...
export function getTemplateDepotBusCells(template, fieldWidth, fieldHeight) {
    const layout = template.depotBusLayout;
    if (!layout) {
        return { ports: [], sections: [] };
    }
    const cells = { ports: [], sections: [] };
...
function isFacilityAdjacentToTemplateBus(facility, template, fieldState, connectionSide) {
...
function isFacilityAdjacentToBusFacilities(facility, busFacilities, connectionSide) {
    const facilityMinX = facility.x;
    const facilityMaxX = facility.x + facility.width - 1;
    const facilityMinY = facility.y;
    const facilityMaxY = facility.y + facility.height - 1;
...
function isFacilityAdjacentToDepotBus(facility, fieldState, template, busFacilities) {
...
export function applyTemplateValidation(fieldState) {
