...
import { getConnectedEntity } from "./connections.js";
/**
 ...
 */
export function recipeToFlowRates(recipe) {
    const inputs = {};
    const outputs = {};
 ...
 */
export function calculateFacilityThrottleFactor(facility, recipe) {
 ...
 */
export function calculateFacilityInputFlows(facility, fieldState) {
 ...
 */
export function calculateFacilityOutputFlows(facility, recipe, throttle) {
    const flowRates = recipeToFlowRates(recipe);
    const flows = [];
 ...
 */
export function distributeFacilityOutputs(outputs, facility) {
    // Count connected output ports (excluding external ports)
    const connectedOutputPorts = facility.ports.filter(port => port.subType === 'output' &&
        !port.external &&
    ...
    const outputsPerPort = outputs.map(flow => ({
        item: flow.item,
 ...
 */
export function applyPathThroughputLimit(path) {
 ...
 */
export function calculatePathFlows(path, fieldState) {
    ...
    let flowDirection = path.flowDirection ?? 'none';
    let flows = [];
...
function filterFlowsForPathType(path, flows) {
    if (flows.length === 0) {
        return flows;
    }
    const isFluidItem = (itemID) => items[itemID]?.fluid === true;
 ...
 */
export function mergeItemFlows(flows) {
    const flowMap = new Map();
 ...
 */
export function analyzeFacilityFlows(facility) {
    const actualRecipe = facility.actualRecipe ? recipes[facility.actualRecipe] : undefined;
    if (!actualRecipe) {
        return { insufficientFlows: new Set(), bottleneckItem: undefined, overSuppliedFlows: new Set() };
    }
    const insufficientFlows = new Set();
    const overSuppliedFlows = new Set();
    let bottleneckItem;
 ...
 */
export function hasFacilityFlowIssues(facility) {
