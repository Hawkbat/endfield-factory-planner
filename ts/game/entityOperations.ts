...
import { type UserChangesJsonV1, isUserChangesJsonV1 } from "../types/external.ts"

/**
 * Create delete changes for all selected entities
 */
export function createDeleteChanges(
 ...
 */
export function createNudgeChanges(
    selectedIDs: ReadonlySet<string>,
    fieldState: Immutable<FieldState>,
    dx: number,
    dy: number
): UserChange[] {
    const nudgeChanges: UserChange[] = []
 ...
 */
export function createCopyChanges(
    selectedIDs: ReadonlySet<string>,
    fieldState: Immutable<FieldState>,
    offset: { dx: number; dy: number } = { dx: 0, dy: 0 }
): UserChange[] {
    const copyChanges: UserChange[] = []
    
    // Track entity index for creating references
    let facilityIndex = 0
    const facilityRefMap = new Map<string, string>() // old ID -> reference token
    let fixtureIndex = 0
    const fixtureRefMap = new Map<string, string>() // old ID -> reference token
 ...
 */
export function serializeCopyData(changes: UserChange[]): string {
    const data: UserChangesJsonV1 = {
 ...
 */
export function deserializeCopyData(json: string): UserChange[] | null {
