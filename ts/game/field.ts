...
import { applyUserChange } from "./changes.ts"
import { validateFacilityPlacement, validateFixturePlacement, validatePathPlacement } from "./geometry.ts"
import { initializeFacilityPorts, initializeFixtureSides, updateFacilityConnections, updateFixtureConnections, updateAllPathConnections, updatePathConnectionRefs } from "./connections.ts"
import { updateAllFacilityPowerStates, calculatePowerStats } from "./power.ts"
import { updateAllFacilityRecipes } from "./recipes.ts"
import { solveFlowSystem, initializeFlowState, propagateFlowsOneIteration } from "./solver.ts"
import { mergeItemFlows } from "./flows.ts"
import { objectValues, type Immutable } from "../utils/types.ts"
import type { FieldState, UserChange, DebugInfo, ItemFlow, FieldFacility } from "../types/field.ts"
...
export function recalculateFieldState(fieldState: Immutable<FieldState>, changes: UserChange[]): Immutable<FieldState> {
    ...
    const facilitiesWithConnections = currentState.facilities.map(facility =>
        updateFacilityConnections(facility, currentState)
    )
    
    const fixturesWithConnections = currentState.pathFixtures.map(fixture =>
        updateFixtureConnections(fixture, currentState)
    )
    ...
    const pathsWithConnectionRefs = currentState.paths.map(path =>
        updatePathConnectionRefs(path, currentState)
    )
    ...
    const depotInputFlowArrays: Immutable<ItemFlow[]>[] = []
    const depotOutputFlowArrays: Immutable<ItemFlow[]>[] = []
    const worldInputFlowArrays: Immutable<ItemFlow[]>[] = []
...
export function updateFacilityPosition(facility: Immutable<FieldFacility>, x: number, y: number): Immutable<FieldFacility> {
...
export function updateFacilityRotation(facility: Immutable<FieldFacility>, rotation: number): Immutable<FieldFacility> {
...
export function updateFacilityPoweredState(facility: Immutable<FieldFacility>, fieldState: Immutable<FieldState>): Immutable<FieldFacility> {
...
export function updateFacilityPathConnections(facility: Immutable<FieldFacility>, fieldState: Immutable<FieldState>): Immutable<FieldFacility> {
