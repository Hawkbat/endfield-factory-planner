...
import { objectEntries, objectKeys, type Immutable } from "../utils/types.ts"
 ...
 */
export function getInputItemsForFacility(facility: Immutable<FieldFacility>): Set<ItemID> {
...
export function findMatchingRecipe(
    ...
    inputItems: Set<ItemID>,
    warnings?: Array<{ facilityId: string, matchingRecipes: RecipeID[] }>
): RecipeID | null {
    ...
    let matchedRecipe: RecipeID | null = null
    let matchCount = 0
 ...
 */
export function canRecipeActivate(recipe: Immutable<typeof recipes[RecipeID]>, facility: Immutable<FieldFacility>): boolean {
 ...
 */
export function shouldUseJumpStart(facility: Immutable<FieldFacility>): boolean {
 ...
 */
export function updateFacilityRecipe(
    facility: Immutable<FieldFacility>,
    warnings?: Array<{ facilityId: string, matchingRecipes: RecipeID[] }>
 ...
 */
export function updateAllFacilityRecipes(
    fieldState: Immutable<FieldState>
): { state: Immutable<FieldState>, warnings: Array<{ facilityId: string, matchingRecipes: RecipeID[] }> } {
    const warnings: Array<{ facilityId: string, matchingRecipes: RecipeID[] }> = []
    
    const updatedFacilities = fieldState.facilities.map(facility =>
        updateFacilityRecipe(facility, warnings)
    )
