...
import { recalculateFieldState } from "./field.ts"
import { facilities } from "../data/facilities.ts"
import { FacilityID, FieldTemplateID, ItemID, PathTypeID, type FieldTemplate } from "../types/data.ts"
import { resolveFieldTemplate } from "../data/templates.ts"

export function createEmptyState(template: FieldTemplateID | Immutable<FieldTemplate>): Immutable<FieldState> {
...
export function createStateFromChanges(
    changes: UserChange[],
    template: FieldTemplateID | Immutable<FieldTemplate> = FieldTemplateID.WULING_MAIN
): Immutable<FieldState> {
    const emptyState = createEmptyState(template)
    return recalculateFieldState(emptyState, changes)
}

export function createInitialTemplateChanges(template: FieldTemplateID | Immutable<FieldTemplate>): UserChange[] {
    const resolvedTemplate = resolveFieldTemplate(template)
...
export function getSampleFieldChanges(): UserChange[] {
    return [
...
export function createSampleFieldState(): Immutable<FieldState> {
