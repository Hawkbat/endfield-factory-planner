...
export const ROTATE_RIGHT_MAP: Record<Direction, Direction> = {
    'up': 'right',
    'right': 'down',
    'down': 'left',
    'left': 'up',
...
export function getDirectionFromPoints(from: Immutable<[number, number]>, to: Immutable<[number, number]>): Direction | null {
    const [fx, fy] = from
    const [tx, ty] = to
...
export function calculatePathEndpointDirection(path: Immutable<FieldPath>, endpoint: 'start' | 'end'): Direction | null {
