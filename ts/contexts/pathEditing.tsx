...
interface PathEditState {
    ...
    isNewPath: boolean
    pathType: PathTypeID
    committedPoints: [number, number][]
    originalPathPoints: [number, number][]
}

interface PathEditingContextValue {    
    // Utilities
    calculateCardinalPoint: (fixedPoint: [number, number], currentPoint: [number, number]) => [number, number]
    normalizePathPoints: (points: [number, number][]) => [number, number][]
    
    // Handlers
    startPathPlacement: (pathType: PathTypeID) => void
    startPathFromPoint: (point: [number, number], pathType: PathTypeID) => void
    startPathFromPortOrFixture: (entityID: string, position: [number, number], pathType: PathTypeID) => void
    resumePathFromEnd: (pathID: string, fromStart: boolean) => void
    handlePathEditMove: (gridPoint: [number, number]) => void
    handlePathEditClick: (gridPoint: [number, number]) => void
    handlePathEditDoubleClick: (gridPoint: [number, number]) => void
    finishPathAtPortOrFixture: (entityID: string, position: [number, number]) => void
    handlePathSegmentDelete: () => void
    cancelPathEdit: () => void
}
...
interface PathEditingProviderProps {
    children: ReactNode
}

export function PathEditingProvider({ children }: PathEditingProviderProps) {
    ...
    const dragInitialStateRef = useRef<UserChange[]>([])
    ...
    const prevEditModeRef = useRef<EditMode | null>(null)
    useEffect(() => {
        ...
        if (editState.isNewPath) {
            ...
            if (
                editState.committedPoints.length === 0 &&
    ...
    function calculateCardinalPoint(fixedPoint: [number, number], currentPoint: [number, number]): [number, number] {
    ...
    function normalizePathPoints(points: [number, number][]): [number, number][] {
        if (points.length < 2) {
            return points
        }

        const normalized: [number, number][] = [points[0]]
     ...
     */
    function startPathPlacement(pathType: PathTypeID = PathTypeID.BELT) {
     ...
     */
    function startPathFromPoint(point: [number, number], pathType: PathTypeID = PathTypeID.BELT) {
        setEditMode(EditMode.PATH_EDITING)
        pathEditStateRef.current = {
            pathID: null,
            isNewPath: true,
    ...
    function startPathFromPortOrFixture(entityID: string, position: [number, number], pathType: PathTypeID) {
        setEditMode(EditMode.PATH_EDITING)
     ...
     */
    function resumePathFromEnd(pathID: string, fromStart: boolean) {
        ...
        let committedPoints: [number, number][]
        if (fromStart) {
        ...
        pathEditStateRef.current = {
            ...
            pathType: path.type,
            committedPoints,
            originalPathPoints: path.points.map(p => [...p] as [number, number])
        }
    }

    function handlePathEditMove(gridPoint: [number, number]) {
        if (editMode !== EditMode.PATH_EDITING) {
            return
        }
    ...
    function handlePathEditClick(gridPoint: [number, number]) {
        if (editMode !== EditMode.PATH_EDITING) {
            return
        }
    ...
    function handlePathEditDoubleClick(gridPoint: [number, number]) {
        if (editMode !== EditMode.PATH_EDITING) {
            return
        }
    ...
    function finishPathAtPortOrFixture(entityID: string, position: [number, number]) {
        if (editMode !== EditMode.PATH_EDITING) {
            return
        }
    ...
    const value: PathEditingContextValue = {
    ...
    return (
        <PathEditingContext.Provider value={value}>
            {children}
        </PathEditingContext.Provider>
    )
}
