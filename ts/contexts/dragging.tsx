import { createContext, useContext, useRef, type ReactNode, type PointerEvent, type RefObject } from "react"
...
import { tuple } from "../utils/types.ts"
import { createStateFromChanges } from "../game/sampleField.ts"
import { generateFixtureRelocationChanges, validateFixturePlacementOnPath } from "../game/fixtures.ts"
import { updateFixtureConnections } from "../game/connections.ts"

interface DraggingContextValue {
    // Handlers to be used by draggable components
    handleDragStart: (event: PointerEvent<SVGElement>, id: string) => void
    handleDragMove: (event: PointerEvent<SVGElement>) => void
    handleDragEnd: (event: PointerEvent<SVGElement>) => void
...
export function DraggingProvider({ 
    children, 
    svgRef,
    pan,
    zoom,
    ...
    // Drag state
    const dragStartGridPointRef = useRef<[number, number]>([0, 0])
    const dragCurrentOffsetRef = useRef<[number, number]>([0, 0])
    const dragInitialStateRef = useRef<UserChange[]>([])
    const dragActiveIDsRef = useRef<ReadonlySet<string>>(new Set())
    
    function getGridPoint(event: { clientX: number, clientY: number }): [number, number] {
        if (!svgRef.current) {
            return [0, 0]
        }
    ...
    function handleDragMove(
        ...
        const moveChanges: UserChange[] = []
        const activeSelectedIDs = dragActiveIDsRef.current
        
        // Use initial state to avoid cumulative movement
        const initialState = createStateFromChanges(
            dragInitialStateRef.current, 
            fieldState.template
        )
    ...
    function handleDragEnd(event: PointerEvent<SVGElement>) {
        if (editMode !== EditMode.DRAGGING) {
            return
        }
        ...
        for (const id of activeSelectedIDs) {
            ...
            if (fixture) {
                ...
                if (targetPath && connectedPathIDs.has(targetPath.id) && targetValidation) {
                    ...
                    const currentValidation = validateFixturePlacementOnPath(
                        fixture.type,
                        [fixture.x, fixture.y],
                        fixture.rotation,
                        targetPath
                ...
                if (isMovingToUnconnectedPath && targetPath && !isMovingToDifferentSegment) {
                    ...
                    const intermediateState = createStateFromChanges(
                        intermediateChanges,
                        fieldState.template
                    )
    ...
    function cancelDrag() {
        ...
        setFieldState(createStateFromChanges(
            dragInitialStateRef.current, 
            fieldState.template
        ))
    }
